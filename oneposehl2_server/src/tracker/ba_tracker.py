...
class BATracker:
    ...
    def apply_ba(self, kpt2ds, kpt2d3d_ids, kpt2d_fids, kpt3d_list, cams, verbose=False):
        device = 'cuda'
        requires_grad = False
        points = torch.tensor(kpt3d_list, device=device, dtype=torch.float64, requires_grad=requires_grad)
        cam_pose = torch.tensor(cams[:, :6], device=device, dtype=torch.float64, requires_grad=requires_grad)
