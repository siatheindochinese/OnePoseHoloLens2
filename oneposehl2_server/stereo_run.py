...
# import hololens stereo dataset
from src.datasets.stereo_dataset import StereoDataset
...
def main(cfg):
	...
	writer= cv2.VideoWriter('stereo_LF.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (480, 640))
	...
	# load stereo dataset
	default_img_dir = 'data/14092022-160721/images/'
	dataset = StereoDataset(default_img_dir)
	...
	def detect_object(inp, init):
		...
		object_detected = not (bbox == np.array([0, 0, 640, 480])).all() #hardcoded dimensions
	...
	while True:
		...
		L, R = dataset[i]
		inp = L
		image = cv2.cvtColor(inp, cv2.COLOR_GRAY2BGR)
